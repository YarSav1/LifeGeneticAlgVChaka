...
SIZE_UNIT = 6  # размер юнитов в пикселях
# RADIUS_UNIT = 6  # радиус у юнита во сколько раз от его размера
LEN_LIFE = 120  # Продолжительность жизни юнита в месяцах
STARVING = 0  # до какой степени юнит может голодать, прежде чем умрет

ONE_YEAR = 5  # число обработок равному 1 игровому году.

ENERGY_FOOD = 50  # сколько энергии будет давать 1 еда
SIZE_FOOD = 3 # Размер еды
SPAWN_START_FOOD = 5000  # сколько еды генерировать в начале
SPAWN_FOOD = 200  # сколько еды генерировать каждый год
...
CHANCE_MUTATION = 25  # вероятность мутации в процентах.
